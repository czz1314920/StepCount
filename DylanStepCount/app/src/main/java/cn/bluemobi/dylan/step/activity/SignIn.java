    ...
    private void addListener() {
        Login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                stuId = AccountNumber.getText().toString();
                stuPsw = LoginPassword.getText().toString();
                String code = et_code.getText().toString();
                if (!code.isEmpty()) codeFlag=true;
                OkHttpUtils.post()
                        .url(loginUrl)
                        ...
                        .addHeader("Referer", "http://jwxw.gzcc.cn/")
                        .build()
                        .connTimeOut(5000)
                            ...
                            public void onResponse(String response) {
                                if (response.contains("验证码不正确")) {
                                    ...
                                    Toast.makeText(getApplicationContext(), "用户名不存在",
                                            Toast.LENGTH_SHORT).show();
                                } else if (codeFlag && !response.contains("用户名不存在") && !response.contains("密码错误") && !response.contains("验证码不正确")){
